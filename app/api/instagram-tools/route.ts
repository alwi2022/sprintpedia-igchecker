// app/api/instagram-tools/route.ts
...
export async function POST(req: NextRequest) {
    try {
        ...
        const setCookies = getLogin.headers.getSetCookie();

        const csrfMatch = setCookies?.join(";").match(/csrf_cookie=([^;]+)/);
        ...
        form.append("username", process.env.SPRINTPEDIA_USERNAME); 
        form.append("password", process.env.SPRINTPEDIA_PASSWORD); 
        ...
        const ciMatch = loginCookies?.join(";").match(/ci_session=([^;]+)/);
        ...
        const isEmptyObj =
            !!root && typeof root === "object" && !Array.isArray(root) && Object.keys(root).length === 0;
